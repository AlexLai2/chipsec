...
""".format( self.BitLockerGUID0, self.BitLockerGUID1, self.BitLockerGUID2, self.BitLockerGUID3[:2].encode('hex').upper(), self.BitLockerGUID3[-6::].encode('hex').upper(), self.FVEMetadataBlock1Offset, self.FVEMetadataBlock2Offset, self.FVEMetadataBlock3Offset )
...
96   NextCounter             : {:08X}
100  Algorithm               : {:08X}
104  Timestamp               : {:016X}""".format( self.Signature, self.Size, self.Version, self.Unknown1, self.Unknown2, self.EncryptedVolumeSize, self.Unknown3, self.VolumeHeaderSectorsNum, self.FVEMetadataBlock1Offset, self.FVEMetadataBlock2Offset, self.FVEMetadataBlock3Offset, self.VolumeHeaderOffset, self.SizeMinusHeader, self.Unknown4, self.Unknown5, self.SizeMinusHeader2, self.VolumeGUID0, self.VolumeGUID1, self.VolumeGUID2, self.VolumeGUID3[:2].encode('hex').upper(), self.VolumeGUID3[-6::].encode('hex').upper(), self.NextCounter, self.Algorithm, self.Timestamp )
...
0x4  ValueType    : {:04X} ({})
0x6  Version (=1) : {:04X}
0x8  Data         :""".format( self.EntrySize, self.EntryType, entry_type, self.ValueType, value_type, self.Version )
...
class VMK( namedtuple('VMK', 'KeyGUID0 KeyGUID1 KeyGUID2 KeyGUID3 LastModified Unknown ProtectionType') ):
  ...
  24  Unknown        : {:04X}
  26  ProtectionType : {:04X} ({})
  28  VMK Data       :
""".format( self.KeyGUID0, self.KeyGUID1, self.KeyGUID2, self.KeyGUID3[:2].encode('hex').upper(), self.KeyGUID3[-6::].encode('hex').upper(), self.LastModified, self.Unknown, self.ProtectionType, protection_type )
...
def parse_bitlocker_volume_header( buf ):
    if not (type(buf) == str):
        logger().error('Invalid object type {}'.format(type(buf)))
        return

    pos = buf.find( BL_FS_SIG )
    if (-1 == pos or pos < 3):
        logger().error( "Valid BitLocker Volume is not found(should have signature '{}' at offset 3): offset = {:x}".format(BL_FS_SIG,pos) )
        return None

    vol_off = pos - 3
    vol = buf[ vol_off : ]
    logger().log( '[bitlocker] Valid BitLocker Volume found at offset 0x{:08X}'.format(vol_off) )
...
def parse_fve_metadata_entires( fve_metadata_entires, size, bLast ):
    ...
    logger().log( "Total size of FVE entries: 0x{:X}".format(size) )
    off = 0
    while off < size:
...
def parse_bitlocker_metadata_block( buf ):
    ...
    fve_off = pos
    fve = buf[ fve_off : ]
    logger().log( '[bitlocker] Valid BitLocker FVE metadata block found at offset 0x{:08X}'.format(fve_off) )
...
def get_recovery_key( recovery_pwd ):
    ...
    for gr in rpwd_groups:
        ...
        k = g//11
        distilled_rkey += ( "{:02X}{:02X}".format(k&0xFF, (k>>8)&0xFF) )
...
def get_rk_password( rkey ):
    ...
    pwd = "{:06d}-{:06d}-{:06d}-{:06d}-{:06d}-{:06d}-{:06d}-{:06d}".format(g0 * 11, g1 * 11, g2 * 11, g3 * 11, g4 * 11, g5 * 11, g6 * 11, g7 * 11)
    return pwd
...
def decrypt_rkey(blob, key):
    ...
    blen = len(blob)
    if blen != 0x38:
        logger().log( " *** Invalid blob length: {:d} ***".format(blen) )
        return ""
    nonce = blob[:12]
    initcounter(nonce)
    ct = blob[12:]

...
def parse_me_region( data ):
    ...
    while (fof + FPTHeaderVer2_size) < len(data):
        ...
        logger().log( '---------------------------------------------------------------------------------------------------' )
        logger().log( 'HeaderMarker  : 0x{:08X} ({})'.format(HeaderMarker, struct.pack('I',HeaderMarker) ) )
        logger().log( 'NumFptEntries : {:d}'.format(NumFptEntries) )
        logger().log( 'HeaderVersion : 0x{:02X}'.format(HeaderVersion) )
        logger().log( 'EntryVersion  : 0x{:02X}'.format(EntryVersion) )
        logger().log( 'HeaderLength  : 0x{:02X}'.format(HeaderLength) )
        logger().log( 'HeaderChecksum: 0x{:02X}'.format(HeaderChecksum) )
        logger().log( 'FileSysData   : 0x{:08X}'.format(FileSysData) )
        logger().log( 'UmaSize       : 0x{:08X}'.format(UmaSize) )
        logger().log( 'Flags         : 0x{:08X}'.format(Flags) )
        fptof = fof
        fof = fof + FPTHeaderVer2_size
    ...
    while ((fof + FPTEntry_size) < len(data)) and (c < NumFptEntries):
        ...
        logger().log( ' {:<11s} | {:<5s} | {:08X} | {:08X} | {:08X}      | {:08X}  | {:08X}       | {:08X}'.format(part_name,part_owner,offset,length,TokensOnStart,MaxTokens,ScratchSectors,attribs) )
...
def parse_me_region_from_file( filename, outpath=None ):
    ...
    for pk in partitions.keys():
        ...
        off = p[2]
        if (off > 0):
            write_file( os.path.join( outpath, "{:08X}-{:08X}-{}.bin".format(off, len(pd), pk) ), pd )

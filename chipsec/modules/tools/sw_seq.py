...
class sw_seq(BaseModule):
    ...
    def display_OPCODES(self, _opmenu_reg, _optype_reg):
        ...
        self.logger.log( '----------------------------' )
        self.logger.log( '' )
        self.logger.log( 'OPMENU: 0x{:016X}'.format(_opmenu_reg) )
        self.logger.log( 'OPTYPE: 0x{:08X}'.format(_optype_reg) )
        self.logger.log( '' )
        self.logger.log( '-------------------------' )
        self.logger.log( 'Opcode #  | Opcode | Type' )
        self.logger.log( '-------------------------' )
        for index in range(8):
            self.logger.log( ' Opcode{:d}  :  0x{:02X}  -  {:d} '.format(index,((_opmenu_reg >> index*8) & 0xFF), ((_optype_reg >> index*2) & 0x3)) )
        self.logger.log( '' )
    ...
    def check_sw_seq(self):
        ...
        if self.logger.VERBOSE:
            self.logger.log_important( 'Detected Max erase size: 0x{:X}'.format(max_erase_size) )
            self.logger.log( '' )
        ...
        if opmenu:
            for op in range(len(OPMENU_OPS)):
                if OPMENU_OPS[op] in BADCODES:
                    badopcode = True
                    self.logger.log_important( 'Unsafe OPCODE : OPCODE'+str(op)+' = 0x{:02X}'.format(OPMENU_OPS[op]) )
                elif OPMENU_OPS[op] not in OPCODES:
                    warning = True
                    self.logger.log_important( 'Unknown OPCODE: OPCODE'+str(op)+' = 0x{:02X}'.format(OPMENU_OPS[op]) )
            ...
            if badopcode:
                self.logger.log_bad( 'Unsafe OPCODEs found' )
                RES.append( 'FAILED' )
            elif warning:
                RES.append( 'WARNING' )
            else:
                self.logger.log_good( 'No unsafe OPCODEs found' )
            if self.logger.VERBOSE:
                codes = ''.join('0x{:02X} '.format(x) for x in OPCODES.keys())
                self.logger.log( '    INFORMATION: Known OPCODEs  - '+codes )
                codes = ''.join('0x{:02X} '.format(x) for x in BADCODES)
        ...
        if opmenu:
            for op in range(len(OPMENU_OPS)):
                if (OPCODES.get(OPMENU_OPS[op], OPTYPES[op]) != OPTYPES[op]):
                    ...
                    self.logger.log_important( msg.format(OPMENU_OPS[op], OPTYPES[op]) )
                    res = ModuleResult.FAILED
            if ( res == ModuleResult.PASSED ):
                self.logger.log_good( 'OPTYPES matched OPCODEs' )
            else:
                self.logger.log_bad( 'OPTYPES did not match OPCODEs' )
                RES.append( 'FAILED' )
        else:
            self.logger.log_important( 'OPMENU contains all 0x00 OPCODES.' )
            self.logger.log_important( 'OPTYPE tests skipped' )
        ...
        if opmenu:
            ...
            if preop0 and (preop0 not in PREFIX):
                self.logger.log_important( 'PREFIX detected: PREFIX0 = 0x{:02X}'.format(preop0) )
                res_fail = True
            if preop1 and (preop1 not in PREFIX):
                self.logger.log_important( 'PREFIX detected: PREFIX1 = 0x{:02X}'.format(preop1) )
                res_fail = True
            if res_fail:
                self.logger.log_bad( 'Found non-whitelisted PREFIX' )
                RES.append( 'FAILED' )
            elif not preop0 or not preop1:
                self.logger.log( '    PREFIX0 = 0x{:02X}'.format(preop0) )
                self.logger.log( '    PREFIX1 = 0x{:02X}'.format(preop1) )
                ...
                self.logger.log_good( 'Only whitelisted PREFIXs found' )
            if self.logger.VERBOSE:
                codes = ''.join('0x{:02X} '.format(x) for x in PREFIX)
                self.logger.log( '    INFORMATION: Known PREFIXs - '+codes )
        else:
            self.logger.log_important( 'OPMENU contains all 0x00 OPCODES.' )
            self.logger.log_important( 'PREFIX tests skipped' )
        ...
        if fpr_enabled:
            ...
            for pr in range(len(PR_REG)):
                ...
                if pr_enabled:
                    ...
                    if self.check_module_size( max_erase_size, base, limit ):
                        ...
                        self.logger.log( '    Reported region Base  : 0x{:08X}'.format(base << 12) )
                        self.logger.log( '    Reported region Limit : 0x{:08X}'.format((limit << 12) + 0xFFF) )
                        self.logger.log( '    Region size           : 0x{:08X}'.format(self.region_size( base, limit )) )
                        res = ModuleResult.FAILED
            ...
            if res == ModuleResult.PASSED:
                self.logger.log_good( 'All PR region size and alignment tests passed' )
            else:
                self.logger.log_important( 'Detected Max erase size: 0x{:X}'.format(max_erase_size) )
                self.logger.log_bad( 'One or more PR region size and alignment tests failed' )
                RES.append( 'FAILED' )
        else:
            self.logger.log_important( 'No PR regions enabled' )
            self.logger.log_important( 'Region size and alignment tests skipped' )


        if 'FAILED' in RES:
            self.logger.log_failed_check( 'SW sequencing SPI configuration invalid' )
            return ModuleResult.FAILED
        elif 'WARNING' in RES:
            self.logger.log_warn_check( 'Unable to determine SW sequencing SPI configuration' )
            return ModuleResult.WARNING
        else:
            self.logger.log_passed_check( 'SW sequencing SPI configuration is valid' )
    ...
    def run(self, module_argv):
        ...
        if len(module_argv) > 0:
            ...
            if ich not in ICH:
                self.logger.log_important( 'Unknown mode: {}'.format(module_argv[0]) )
                self.logger.log(__doc__)
                return ModuleResult.ERROR
            else:
                self.logger.log_important( 'Bypassing configuration files and using predefined ICH settings: {}'.format(ich) )

        self.res = self.check_sw_seq()
        return self.res

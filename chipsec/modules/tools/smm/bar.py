...
class bar(BaseModule):
    ...
    def check_mmio_noSMI(self):
        for bar_name in self.bars.keys():
            self.logger.log("[*] '{}' normal difference ({:d} diffs):".format(bar_name,3*NO_DIFFS))
            self.logger.flush()
            diff = []
            for i in range(3*NO_DIFFS):
                ...
                self.logger.log("    diff{:d}: {:d} regs {}".format(i,len(d),d))
                diff = list(set(diff)|set(d))
            self.bars_diff[bar_name] = diff
            self.logger.log("    {:d} regs changed: {}".format(len(diff),sorted(diff)))

    def read_BARs(self):
        bars = {}
        for bar_name in self.bar_names:
            self.logger.log("    reading '{}'".format(bar_name))
        ...
        for smi_code in range(self.smic_start,self.smic_end+1):
            ...
            for smi_data in range(self.smid_start,self.smid_end+1):
                for ecx in range(self.smif_start,self.smif_end+1):
                    self.logger.log("[*] SMI# {:02X}: data {:02X}, func (ECX) 0x{:08X}".format(smi_code,smi_data,ecx) )
                    bars = self.read_BARs()
    ...
    def run( self, module_argv ):
        ...
        self.logger.log("    MMIO BAR names: {}".format(self.bar_names))
        self.logger.log("    Generate SMI: {}".format('True' if self.generate_smi else 'False'))
        self.logger.log("    SMI codes: [0x{:02X}:0x{:02X}]".format(self.smic_start,self.smic_end))

        # allocate a page or SMM communication buffer (often supplied in EBX register)
        (va, self.comm) = self.cs.mem.alloc_physical_mem( 0x1000, BOUNDARY_4GB-1 )
        self.logger.log( "[*] SMM comm buffer (EBX)  : 0x{:016X}".format(self.comm) )
        ...
        for bar_name in self.bar_names:
            if self.cs.mmio.is_MMIO_BAR_defined(bar_name):
                ...
                self.logger.log("[*] MMIO BAR '{}': base = 0x{:016X}, size = 0x{:08X}".format(bar_name,base,size))
            else:
                self.bar_names.remove(bar_name)
                self.logger.warn("'{}' BAR is not defined. ignoring..".format(bar_name))

        self.logger.log("[*] reading contents of MMIO BARs {}".format(self.bar_names))

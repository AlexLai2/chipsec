...
class tcoctl_lock(BaseModule):

    def __init__(self):
        BaseModule.__init__(self)

    def is_supported(self):
        if self.cs.is_control_defined('TcoCtlLock'):
            return True
        else:
            self.res = ModuleResult.NOTAPPLICABLE
            return False

    def check_tco_lock(self):
        self.logger.start_test( "TCO Control Lock" )

        tco_base_lock = self.cs.get_control('TcoCtlLock', with_print=True)
        if(tco_base_lock):
            self.logger.log_passed_check( 'The lock bit for the TCOBASE is set' )
            return ModuleResult.PASSED
        self.logger.log_failed_check( 'The lock bit for the TCOBASE was not set' )
        return ModuleResult.FAILED

    def run( self, module_argv ):
        self.res = self.check_tco_lock()
        return self.res

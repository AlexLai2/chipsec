...
from chipsec.module_common import BaseModule, ModuleResult, MTAG_BIOS, MTAG_SECUREBOOT, OPT_MODIFY
from chipsec.hal.uefi import UEFI, get_auth_attr_string
from chipsec.hal.uefi_common import EFI_VARIABLE_RUNTIME_ACCESS, EFI_VARIABLE_NON_VOLATILE
from chipsec.hal.uefi_common import IS_EFI_VARIABLE_AUTHENTICATED, IS_VARIABLE_ATTRIBUTE, EFI_VARIABLE_BOOTSERVICE_ACCESS
...
class access_platform(BaseModule):
    ...
    def can_modify(self, name, guid, data):
        ...
        if self.diff_var(newdata, origdata):
            ret = True
            self.logger.log_bad('EFI variable {} has been corrupted. Recovering..'.format(name))
            self._uefi.set_EFI_variable(name, guid, origdata)
            if self.diff_var(self._uefi.get_EFI_variable(name, guid), origdata):
                nameguid = name+' {'+guid+'}'
                self.logger.error('RECOVERY FAILED. Variable {} remains corrupted. Original data value: {}'.format(nameguid, origdata))
        return ret
    ...
    def modify_variables( self, variables ):
        ...
        for (name, guid, data) in variables:
            if self.can_modify( name, guid, data ):
                ...
                self.logger.log_bad( '  {:<26s}: HAS BEEN MODIFIED'.format(name) )
            else:
                self.logger.log_good( '  {:<26s}: could not modify'.format(name) )
        if len(modified_concern) > 0:
            res = True
            self.logger.log_bad('The following variables of concern can be modified:')
            for name in modified_concern: self.logger.log('    {}'.format(name))
        return res
    ...
    def check_vars(self, do_modify):
        ...
        for name in vars.keys():
            ...
            if len(vars[name]) > 1: self.logger.log_important( 'Found two instances of the variable {}.'.format(name) )
            ...
            for (off, buf, hdr, data, guid, attrs) in vars[name]:
                ...
                str_auth = ('auth ({})'.format(get_auth_attr_string(attrs))) if is_auth else ''
                ...
                self.logger.log( " {:<30s} | {:<12s} | {:<14s} | {:<16s} | {:<9s} | {}".format(name, str_nv, str_access, str_auth, str_concern_var, str_is_concern) )

...
class rowhammer(BaseModule):
    ...
    def check_antiRH_pTRR( self, device, ptrr_mac ):
        ...
        self.logger.log( "      Pseudo TRR (pTRR)            : 0x{:01X}".format(pTRR) )
        return (pTRR == 1)
    ...
    def check_antiRH_MAC( self, device, ptrr_mac ):
        ...
        self.logger.log( "      Unlimited MAC                : 0x{:01X}".format(umac) )
        mac = (ptrr_mac & _SPD_OFFSET_PTRR_TMAW_MAC_MAC_MASK) >> _SPD_OFFSET_PTRR_TMAW_MAC_MAC_BIT
        self.logger.log( "      Maximum Activate Count (MAC) : 0x{:01X} ({})".format(mac,MAC[mac]) )
        maw = (ptrr_mac & _SPD_OFFSET_PTRR_TMAW_MAC_MAW_MASK) >> _SPD_OFFSET_PTRR_TMAW_MAC_MAW_BIT
        self.logger.log( "      Maximum Active Window (MAW)  : 0x{:01X} ({})".format(maw,MAW[maw]) )
    ...
    def check_ECC( self, device ):
        ...
        self.logger.log( '    {} {} ECC'.format(chipsec.hal.spd.SPD_DIMMS[device],'supports' if ecc_ok else 'does not support')  )
        return ecc_ok
    ...
    def run( self, module_argv ):
        ...
        self.smbus.reset_SMBus_controller()

        _dimms = self.spd.detect()
        self.logger.log( "[*] Found {:d} DIMM(s) with SPD EEPROM".format(len(_dimms)))
        if 0 == len(_dimms):
        ...
        for _dimm in _dimms:
            ...
            self.logger.log( "[*] reading {} SPD information..".format(_dimm_n))
            #self.spd.decode( _dimm )
            dram_type   = self.spd.getDRAMDeviceType( _dimm )
            module_type = self.spd.getModuleType( _dimm )
            self.logger.log( "    {} DRAM device type (byte 2): 0x{:01X} ({})".format(_dimm_n, dram_type, chipsec.hal.spd.dram_device_type_name(dram_type)) )
            self.logger.log( "    {} module type (byte 3)     : 0x{:01X} ({})".format(_dimm_n, module_type, chipsec.hal.spd.module_type_name(module_type)) )

            self.logger.log( "[*] checking {} for row hammer mitigations..".format(_dimm_n) )

            ecc = self.check_ECC( _dimm )

            if (chipsec.hal.spd.DRAM_DEVICE_TYPE_DDR3 != dram_type):
                self.logger.log_good( '{} is not DDR3'.format(_dimm_n) )
                continue

            ptrr_mac = self.spd.read_byte( _SPD_OFFSET_PTRR_TMAW_MAC, _dimm )
            self.logger.log( "    PTRR/TMAW/MAC (byte 0x{:02X})      : 0x{:02X}".format(_SPD_OFFSET_PTRR_TMAW_MAC, ptrr_mac) )
            ptrr_ok = self.check_antiRH_pTRR( _dimm, ptrr_mac )
            mac_sts = self.check_antiRH_MAC( _dimm, ptrr_mac )

            if ptrr_ok:
                self.logger.log_good( "{} has row hammer mitigation (pTRR compliant)".format(_dimm_n) )
            elif MAC_PASS == mac_sts:
                self.logger.log_good( "{} has row hammer mitigation (supports Unlimited MAC)".format(_dimm_n) )
            elif MAC_WARN == mac_sts:
                if ModuleResult.FAILED != res: res = ModuleResult.WARNING
                self.logger.log_important( "{} supports MAC but needs to be tested".format(_dimm_n) )
            elif ecc:
                if ModuleResult.FAILED != res: res = ModuleResult.WARNING
                self.logger.log_important( "{} supports ECC but needs to be tested".format(_dimm_n) )
            else:
                res = ModuleResult.FAILED
                self.logger.log_bad( "{} does not seem to have any row hammer mitigation".format(_dimm_n) )

...
from chipsec.hal.bitlocker import parse_bitlocker_volume_header, parse_bitlocker_metadata_block, get_recovery_key
from chipsec.hal.bitlocker import get_rk_password, decrypt_rkey
...
class BitlockerCommand(BaseCommand):
    ...
    def run(self):
        ...
        if ( 'volume' == op ):
            ...
            pwd = self.argv[ 3 ]
            self.logger.log( "[CHIPSEC] Bitlocker recovery password: {}".format(pwd) )
            self.logger.log( "[CHIPSEC] Distilled recovery key: {}".format(get_recovery_key( pwd )) )
        elif ( 'rpwd' == op ):
            rkey = self.argv[ 3 ]
            import binascii
            b_rkey = binascii.unhexlify( rkey )
            self.logger.log( "[CHIPSEC] Bitlocker recovery key ( {} ):".format(rkey) )
            print_buffer( b_rkey )
            self.logger.log( "[CHIPSEC] Recovery password: {}".format(get_rk_password( b_rkey )) )
        elif ( 'dec_aesccm' == op ):
            blob = read_file( self.argv[ 3 ] )
            key  = read_file( self.argv[ 4 ] )
            cleartext_f = self.argv[ 5 ]
            self.logger.log( "[CHIPSEC] Decrypting AES-CCM encrypted blob to file '{}'".format(cleartext_f) )
            write_file( cleartext_f, decrypt_rkey( blob, key ) )
        else:
            self.logger.error( "unknown command-line option '{:.32s}'".format(op) )
            print (BitlockerCommand.__doc__)
            return

        self.logger.log( "[CHIPSEC] (bitlocker) time elapsed {:.3f}".format(time.time()-t) )
